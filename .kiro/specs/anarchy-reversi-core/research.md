...
### Decision: リセットボタンのモバイル配置
- **Context**: モバイルレイアウトでリセットボタンがボード直下に手番終了ボタンと並んで配置されていた。ゲーム中の誤タップでリセットされるリスクが高い
- **Alternatives Considered**:
  1. ボード直下に手番終了ボタンと並べて配置（変更前の状態）
  2. ヘッダーのボタン群（遊び方・退出の横）に移動
  3. 確認ダイアログを追加
- **Selected Approach**: ヘッダーのボタン群に移動。モバイル・デスクトップ共通で表示
- **Rationale**: ヘッダーのボタンは小さく（text-sm）、ゲーム操作エリアから離れているため誤操作リスクが低い。確認ダイアログは子供向けUIとしてはステップが増えて煩わしい
- **Trade-offs**: リセットの発見しやすさはやや下がるが、誤操作防止を優先
- **Follow-up**: なし

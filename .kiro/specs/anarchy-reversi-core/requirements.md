...
### Requirement 2: 石の自由操作

**Objective:** プレイヤーとして、盤面上の任意のマスをクリックして石の状態を自由に変更したい。ルールに縛られず自由に遊べるようにするため。

#### Acceptance Criteria

1. When プレイヤーが空のセルをクリックした時, the アナーキーオセロ shall そのセルに黒石を配置する
2. When プレイヤーが黒石のセルをクリックした時, the アナーキーオセロ shall そのセルの石を白石に変更する
3. When プレイヤーが白石のセルをクリックした時, the アナーキーオセロ shall そのセルを空にする
4. The アナーキーオセロ shall 合法手の判定を行わず、すべてのセルを操作可能にする
5. The アナーキーオセロ shall ターンの制限を設けず、どちらのプレイヤーでもいつでも操作可能にする
6. When セルがクリックされた時, the アナーキーオセロ shall 即座に視覚的なフィードバック（石の反転表示）を提供する
